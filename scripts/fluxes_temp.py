...
vz = np.array(data['z-GasMomentum'])/rho
...
zp1kpc = (np.abs(Z))<(zhgt + delta*dz)
zm1kpc = (np.abs(Z))<(zhgt - delta*dz)
z1kpc = np.ma.mask_or(zp1kpc, zm1kpc)
z1kpc = zp1kpc ^ zm1kpc
zcap  = (vz*zrange>0.0)
out_flux = z1kpc * zcap
...
mass_outflow_1kpc = (rho * vz * dA)
massflux, binx = np.histogram(temp[out_flux].reshape(-1,1)[:,0], 
                        weights=np.abs(mass_outflow_1kpc[out_flux]).reshape(-1,1)[:,0], bins=tedges)

metal_outflow_1kpc = rhoOxy_inj *  vz * dA
metflux, binx = np.histogram(temp[out_flux].reshape(-1,1)[:,0], 
                    weights=np.abs(metal_outflow_1kpc[out_flux]).reshape(-1,1)[:,0], bins=tedges)

total_met_outflow = 8.6e-3*mass_outflow_1kpc + metal_outflow_1kpc
metflux_zsol, binx = np.histogram(temp[out_flux].reshape(-1,1)[:,0], 
                    weights=np.abs(total_met_outflow[out_flux]).reshape(-1,1)[:,0], bins=tedges)
...
hfo.create_dataset('TempBins'       , data=binx)
hfo.create_dataset('MassFlux'       , data=massflux)
hfo.create_dataset('MetalFlux'       , data=metflux)
hfo.create_dataset('MetalFluxZbg'       , data=metflux_zsol)
