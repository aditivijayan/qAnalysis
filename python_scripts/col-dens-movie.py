...
plt.rcParams['xtick.major.width']=2.1
...
def makeSlicePlot(queue):
    while True:
        ...
        fig, ax = plt.subplots(1, 1, gridspec_kw = {'wspace':0.00, 'hspace':0.0},figsize=(12, 48))
        ...
        plot = ax.pcolormesh(xrange/kpc,zrange/kpc, np.transpose(tot_rho),\
                    norm=mcolors.LogNorm(vmin=1.e-6, vmax=1.e-2),
        ...
        ax.text(0.05,3.8, '%.1f'%(curr_timestep) + ' Myr', color='white')

        ax.tick_params(axis='x', labelbottom=False, bottom=False)
        plt.setp(ax, 'ylim', (0.0, 4.0))


        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
                
        outputfile_name =os.path.join(output_folder, 'cold-mov-' + f.split('proj_y_plt')[-1].split('/')[0] + '.jpeg')
